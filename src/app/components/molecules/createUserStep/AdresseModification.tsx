...
import { setModificationDetails } from "@/redux/createUserSlice";
...
const AdresseModification = ({
  ...
  const [formValues, setFormValues] = useState<{
    ...
    dateDeChangement: modificationData.dateDeChangement || "",
    CGV: data.CGV || false,
    activite: "",
    activiteDetail: "",
    categorie: "",
  ...
  const handleChange = (
    ...
    const { name, type, value, checked } = e.target as HTMLInputElement; // Casting pour accéder à `checked`
    let updatedValue = type === "checkbox" ? checked : value;

    // Mise à jour des valeurs du formulaire avec la nouvelle valeur mise à jour
    setFormValues((prev) => ({ ...prev, [name]: updatedValue }));

    // Réinitialisation de l'état des erreurs pour le champ courant
    setFormErrors((prevErrors) => ({
      ...prevErrors,
      [name]: "",
    }));

    // Validation spécifique pour le numéro de SIRET
    if (
      name === "siret" &&
      typeof updatedValue === "string" &&
      !siretRegex.test(updatedValue)
    ) {
      setFormErrors((prevErrors) => ({
        ...prevErrors,
    ...
    // Ajouter ici la validation pour d'autres champs si nécessaire
    // Par exemple, validation de l'email
    if (
      name === "email" &&
      typeof updatedValue === "string" &&
      ...
      setFormErrors((prevErrors) => ({
        ...prevErrors,
        [name]: "Adresse email invalide.",
      }));
    }

    // Validation pour s'assurer que le champ activité est sélectionné
    if (name === "activite" && updatedValue === "") {
      setFormErrors((prevErrors) => ({
        ...prevErrors,
        [name]: "Veuillez sélectionner une activité.",
      }));
    }
  ...
  useEffect(() => {
    dispatch(setModificationDetails(formValues));
  }, [formValues, dispatch]);
      ...
      <>
        ...
        {(formValues.activite === "Je ne trouve pas mon domaine d’activité" ||
          ...
          <>
            ...
            <div className="w-full mt-5">
              <label htmlFor="categorie" className="text-slate-700 text-sm">
...
export default AdresseModification;
