...
const CheckoutModification = () => {
  const dispatch = useDispatch();
  const router = useRouter();

  const userNom = useSelector(
    (state: RootState) => state.createUser.userModification.nom
  );
  const userPrenom = useSelector(
    (state: RootState) => state.createUser.userModification.prenom
  );

  const siret = useSelector(
    (state: RootState) => state.createUser.userModification.siret
  );
  ...
  if (!siret || !typeDeModification || !userNom || !userPrenom) {
  ...
  }, [siret, typeDeModification, userNom, userPrenom]);
          ...
          {open && (
            <div className="p-3 text-start w-[90%] mx-auto text-slate-400 text-sm">
              ...
              <h3 className="mt-1.5">
                ...
                <span className="font-semibold">{typeDeModification}</span>
              </h3>
              <h3 className="mt-1.5">
...
export default CheckoutModification;
