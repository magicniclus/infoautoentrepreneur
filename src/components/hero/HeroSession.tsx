...
import { ajouterCessionUtilisateur } from "@/firebase/database/database";
import { initializeCompte, setCessationDetails } from "@/redux/createUserSlice";
...
const HeroCession = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { email, siret } = formValues;

    const dossierId = Math.random().toString(36).substr(2, 9); // Génération d'un ID de dossier unique

    try {
      await ajouterCessionUtilisateur(
        "brouillonCession", // Ce paramètre peut varier selon votre implémentation
        dossierId,
        email,
        siret
      );
      ...
      router.push("/cessation/declaration"); // Assurez-vous d'avoir défini 'router'
    } catch (error) {
  ...
  return (
    <>
      ...
      <section
        id="form"
        className="w-full bg-[url('/background/cession.jpg')] py-10 md:py-24 bg-no-repeat bg-cover bg-center relative background text-slate-700"
      >
          ...
          <div className="md:w-1/2 w-full">
            ...
            <ul className=" text-white mt-8">
              <li className="mt-3 flex items-center  text-normal">
                ...
                Cession de votre auto-entreprise
              </li>
              <li className="mt-3 flex items-center text-normal">
                <div className="flex justify-center items-center">
                  <CheckCircleIcon className="min-w-[12px] h-7 w-auto text-white mr-3" />
          ...
          <div className="md:w-1/2 w-full flex justify-center">
            ...
            >
              <h2 className="text-center font-semibold w-9/12 mx-auto">
                Formulaire de cession d&apos;activité simplifié
              </h2>
